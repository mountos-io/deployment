resource "google_compute_instance_template" "s3gatewayserv" {
  count        = var.s3gateway_enable ? 1 : 0
  name_prefix  = "mountos-s3gatewayserv-"
  machine_type = var.s3gateway_machine_type
  ...
  network_interface {
    ...
    access_config {} # ephemeral external IP: s3gatewayserv advertises a public IPv4
  }

  service_account {
    email = google_service_account.s3gatewayserv[0].email
  ...
  metadata = {
    startup-script = templatefile("${path.module}/s3gateway-cloud-init.s3gatewayserv.sh.tftpl", {
...
resource "google_compute_health_check" "s3gatewayserv" {
  count = var.s3gateway_enable ? 1 : 0
...
resource "google_compute_region_instance_group_manager" "s3gatewayserv" {
  count              = var.s3gateway_enable ? 1 : 0
  name               = "mountos-s3gatewayserv"
  region             = var.region
  base_instance_name = "mountos-s3gatewayserv"
  target_size        = var.s3gateway_count

  version {
    instance_template = google_compute_instance_template.s3gatewayserv[0].id
  }

  distribution_policy_zones = local.zones

  auto_healing_policies {
    health_check      = google_compute_health_check.s3gatewayserv[0].id
  ...
  depends_on = [
    ...
    google_secret_manager_secret_iam_member.s3gatewayserv_secret_id_reader,
    google_secret_manager_secret_iam_member.s3gatewayserv_vault_ca_reader,
  ]
}
