resource "azurerm_public_ip_prefix" "s3gatewayserv" {
...
resource "azurerm_linux_virtual_machine_scale_set" "s3gatewayserv" {
  count               = var.s3gateway_enable ? 1 : 0
  name                = "mountos-s3gatewayserv"
  resource_group_name = azurerm_resource_group.main.name
  location            = azurerm_resource_group.main.location
  sku                 = var.s3gateway_vm_size
  instances           = var.s3gateway_count
  zones               = var.zones
  ...
  identity {
    type         = "UserAssigned"
    identity_ids = [azurerm_user_assigned_identity.s3gatewayserv[0].id]
  }
  ...
  network_interface {
    ...
    ip_configuration {
      ...
      public_ip_address {
        name                = "public"
        public_ip_prefix_id = azurerm_public_ip_prefix.s3gatewayserv[0].id
      }
    }
  }

  custom_data = base64encode(templatefile("${path.module}/s3gateway-cloud-init.s3gatewayserv.sh.tftpl", {
    ...
    identity_client_id      = azurerm_user_assigned_identity.s3gatewayserv[0].client_id
  ...
  depends_on = [
    ...
    azurerm_role_assignment.s3gatewayserv_ca_reader,
    azurerm_role_assignment.s3gatewayserv_secret_id_reader,
    azurerm_role_assignment.s3gatewayserv_secretstore,
  ]
}
