...
data "aws_iam_policy_document" "appserv_secret_id" {
  statement {
    ...
    resources = [
      "arn:aws:ssm:${var.region}:${data.aws_caller_identity.current.account_id}:parameter/${local.name_root}/appserv/vault-secret-id",
      "arn:aws:ssm:${var.region}:${data.aws_caller_identity.current.account_id}:parameter/${local.name_root}/hub/vault-ca",
    ]
  }
  statement {
    actions   = ["kms:Decrypt"]
...
resource "aws_iam_role_policy" "appserv_secret_id" {
  name   = "${local.name_root}-appserv-secret-id"
...
# <name_root>/api-master (region-only key material), and region services can
# never read <name_root>/appserv (hub signing key + admin DSN + dashboard HMAC).
# Secret ARNs carry a random 6-char suffix, hence the -?????? matches (exactly
# six, so a hostile <name_root>/appserv-evil name cannot ride along); the
# s3creds/volcreds path wildcards intentionally cover their dynamic per-volume names.
locals {
  ...
  appserv_secret_arns = [
    "${local.sm_arn_prefix}:${local.name_root}/appserv-??????",
    "${local.sm_arn_prefix}:${local.name_root}/service-verifiers-??????",
  ]
  ...
  dataserv_secret_read_arns = [
    "${local.sm_arn_prefix}:${local.name_root}/dataserv-??????",
    "${local.sm_arn_prefix}:${local.name_root}/gcserv-??????",
    "${local.sm_arn_prefix}:${local.name_root}/service-verifiers-??????",
    "${local.sm_arn_prefix}:${local.name_root}/api-master-??????",
    "${local.sm_arn_prefix}:${local.name_root}/s3creds/*",
    "${local.sm_arn_prefix}:${local.name_root}/volcreds/*",
  ]
  dataserv_secret_write_arns = [
    "${local.sm_arn_prefix}:${local.name_root}/api-master-??????", # gcserv rotation
    "${local.sm_arn_prefix}:${local.name_root}/s3creds/*",
    "${local.sm_arn_prefix}:${local.name_root}/volcreds/*",
  ]
  ...
  region_worker_secret_arns = [
    "${local.sm_arn_prefix}:${local.name_root}/blockserv-??????",
    "${local.sm_arn_prefix}:${local.name_root}/service-verifiers-??????",
    "${local.sm_arn_prefix}:${local.name_root}/s3creds/*",
    "${local.sm_arn_prefix}:${local.name_root}/volcreds/*",
  ]
...
resource "aws_iam_role_policy" "appserv_secretstore" {
  ...
  name   = "${local.name_root}-appserv-secretstore"
  role   = aws_iam_role.appserv.id
...
resource "aws_iam_role_policy" "dataserv_secretstore" {
  ...
  name   = "${local.name_root}-dataserv-secretstore"
